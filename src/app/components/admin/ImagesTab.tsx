...
export const ImagesTab = ({ detail, propertyId, onSaved }: Props) => {
  ...
  const [bulkUnassigningUnit, setBulkUnassigningUnit] = useState(false);
  // inline toggle saving state
  const [togglingImg, setTogglingImg] = useState<string | null>(null);

  const [propImageFlags, setPropImageFlags] = useState<
    Map<string, PropImageFlag>
  >(new Map());
  const [unitImageFlags, setUnitImageFlags] = useState<
    Map<string, UnitImageFlag>
  >(new Map());
  ...
  const togglePropImageFlag = async (
    imageId: string,
    flag: "is_banner_image" | "is_carousel_image",
    value: boolean,
  ) => {
    setTogglingImg(imageId);
    try {
      await httpService().put(
        `/admin/properties/${propertyId}/images/${imageId}`,
        { [flag]: String(value) },
      );
      setPropImageFlags((prev) => {
        const n = new Map(prev);
        if (flag === "is_banner_image" && value) {
          detail.images.forEach((pi) => {
            if (pi.image_id !== imageId) {
              const cur = n.get(pi.image_id) ?? {
                is_banner_image: pi.is_banner_image,
                is_carousel_image: pi.is_carousel_image,
              };
              n.set(pi.image_id, { ...cur, is_banner_image: "false" });
            }
          });
        }
        const cur = n.get(imageId) ?? {
          is_banner_image:
            detail.images.find((pi) => pi.image_id === imageId)
              ?.is_banner_image ?? "false",
          is_carousel_image:
            detail.images.find((pi) => pi.image_id === imageId)
              ?.is_carousel_image ?? "false",
        };
        n.set(imageId, { ...cur, [flag]: String(value) });
        return n;
      });
    } catch {
      setError("Failed to update image settings");
    } finally {
      setTogglingImg(null);
    }
  };

  const toggleUnitImageFlag = async (
    imageId: string,
    flag: "is_banner_image",
    value: boolean,
  ) => {
    if (!selectedUnit) return;
    setTogglingImg(imageId);
    try {
      await httpService().put(
        `/admin/properties/${propertyId}/units/${selectedUnit}/images/${imageId}`,
        { [flag]: String(value) },
      );
      setUnitImageFlags((prev) => {
        const n = new Map(prev);
        const unitImgs =
          detail.units.find((u) => u.unit_id === selectedUnit)?.images ?? [];
        if (flag === "is_banner_image" && value) {
          unitImgs.forEach((ui) => {
            if (ui.image_id !== imageId) {
              const cur = n.get(ui.image_id) ?? {
                is_banner_image: ui.is_banner_image,
              };
              n.set(ui.image_id, { ...cur, is_banner_image: "false" });
            }
          });
        }
        const cur = n.get(imageId) ?? {
          is_banner_image:
            unitImgs.find((ui) => ui.image_id === imageId)?.is_banner_image ??
            "false",
        };
        n.set(imageId, { ...cur, [flag]: String(value) });
        return n;
              ...
              pi.image && (
                <Box
                  ...
                  sx={{
                    ...
                    cursor: propUnassignMode ? "pointer" : "default",
                    opacity:
                      removing === pi.image_id || togglingImg === pi.image_id
                        ? 0.4
                        : 1,
                    outline: propUnassignSel.has(pi.image_id)
                      ? "2px solid"
                ...
                >
                  ...
                  {/* Top-left: remove chip in bulk mode, or Banner/Carousel toggles */}
                  <Box
                    sx={{
                      position: "absolute",
                      top: 2,
                  ...
                  >
                    ...
                    {/* Inline toggles — only shown when not in bulk-remove mode */}
                    {!propUnassignMode && (
                      <Box
                        onClick={(e) => e.stopPropagation()}
                        sx={{
                        ...
                        <FormControlLabel
                          ...
                          control={
                            <Switch
                              size="small"
                              checked={
                                (propImageFlags.get(pi.image_id)
                                  ?.is_banner_image ?? pi.is_banner_image) ===
                                "true"
                              }
                              disabled={togglingImg === pi.image_id}
                              onChange={(e) =>
                                togglePropImageFlag(
                                  pi.image_id,
                                  "is_banner_image",
                              ...
                              <FormControlLabel
                                ...
                                control={
                                  <Switch
                                    size="small"
                                    checked={
                                      (unitImageFlags.get(ui.image_id)
                                        ?.is_banner_image ??
                                        ui.is_banner_image) === "true"
                                    }
                                    disabled={togglingImg === ui.image_id}
                                    onChange={(e) =>
                                      toggleUnitImageFlag(
                                        ui.image_id,
                                        "is_banner_image",
